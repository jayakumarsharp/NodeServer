...
var EstimationEGainModel = {

    //Get PricesheetbyOppId
    GetEstimationEGain: function (oppId) {
        var deferred = q.defer();
        try {
            logger.info('price sheet called')
            if (oppId != null && oppId != undefined) {
                models.EstimationEGain.findAll({ where: { OppId: oppId } })
            ...
            models.OppApplicationMap.findAll({ where: { ApplicationId: GroupId, OppId: OppId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        ...
                        models.OppApplicationMap.findAll({ where: { OppId: OppId, SheetName: 'EGainSheet' } })
                            .then(function (opp) {
                                if (opp.length > 0) {
                                    ...
                                    models.sequelize.transaction().then(function (t) {
                                        if (t != null && t != undefined) {
                                            sequelize.query('insert into TBL_EstimationEGain (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,GroupId) select :OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,:groupid from TBL_EstimationEGainMaster EM left outer join TBL_EstimationEGain EA on EM.RowId=EA.Rowid and (EA.GroupId = :groupid or EA.GroupId is null)', { replacements: { OppId: OppId, groupid: GroupId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                                .then(function () {
    ...
    GetMaximumGroupEstimationEGainId: function () {
        var deferred = q.defer();
        try {
    ...
    AddEstimationEGain: function (EstimationModel) {
        var deferred = q.defer();
        try {
            ...
            models.sequelize.transaction().then(function (t) {
                if (t != null && t != undefined) {
                    if (EstimationModel.IsSaveAs) {
                        models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId, SheetName: 'EGainSheet', IsEditable: true, IsLocked: true } })
                            .then(function (opp) {
                                if (opp.length > 0) {

                                    if (opp[0].LockedUser == EstimationModel.Authour) {

                                        sequelize.query('update TBL_EstimationOppMapping set IsEditable=0,IsLocked=0,LockedIn=NULL,LockedUser=NULL,LockStartTime=NULL where OppId = :id and SheetName=:sheetname', { replacements: { id: EstimationModel.OppId, sheetname: "EGainSheet" }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                            .then(function () {
                                                models.OppApplicationMap.create({
                                                    OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'EGainSheet'
                                                }, { transaction: t })
                                                    .then(function () {
                        ...
                        models.OppApplicationMap.update({
                            ...
                            .then(function () {
                                models.EstimationEGain.destroy({ where: { GroupId: EstimationModel.ApplicationId }, truncate: false }, { transaction: t })
                                    .then(function () {
                                        Addinternalcall(EstimationModel, t, deferred);
                                    })
    ...
    AddEstimationEGainNewVersion: function (EstimationModel) {
        var deferred = q.defer();
        try {
                ...
                models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId } })
                    .then(function (opp) {
                        if (opp.length > 0) {
                            ...
                            models.OppApplicationMap.create({
                                OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'EGainSheet', IsInitial: true
                            }, { transaction: t })
                                .then(function () {
                                    sequelize.query('insert into TBL_EstimationEGain (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,GroupId) select EA.OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,:groupid from TBL_EstimationEGainMaster EM left outer join TBL_EstimationEGain EA on EM.RowId=EA.Rowid  and (EA.GroupId = :oldid or EA.GroupId is null)', { replacements: { oldid: EstimationModel.ExisitingPriceGroupId, groupid: EstimationModel.ApplicationId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                        .then(function () {
                                            t.commit();
    ...
    GetAllEstimationEGainbyOppGroupID: function (oppId, ApplicationId) {
        var deferred = q.defer();
        try {
            models.OppApplicationMap.findAll({ where: { ApplicationId: ApplicationId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        if (opp[0].IsEditable) {
                            sequelize.query('SELECT EE.[Id],[OppId],EEM.[GroupType],EEM.[Name],EEM.[col1],EEM.[col2],EEM.[col3],EEM.[Name1],EEM.[Fcol1],EEM.[Fcol2],EEM.[Fcol3],EEM.[RowId],[A1col1],[A1col2],[A1col3],[A1Fcol1],[A1Fcol2],[A1Fcol3],[GroupId]  FROM TBL_EstimationEGainMaster EEM  left outer join [dbo].[TBL_EstimationEGain] EE on EE.RowId =EEM.RowId where GroupId= :groupid or GroupId is null', { replacements: { groupid: ApplicationId }, type: sequelize.QueryTypes.SELECT })
                            ...
                            models.EstimationEGain.findAll({ where: { GroupId: ApplicationId } })
...
module.exports.EstimationEGainModel = EstimationEGainModel;
...
function Addinternalcall(ApplicationModel, t, deferred) {
    try {
        ...
        if (t != null && t != undefined) {
            ...
            models.EstimationEGain.bulkCreate(ApplicationModel.EGainSheet, { transaction: t }, { omitNull: true }).then(function () {
                t.commit();
...
function UpdateFormuleusingSP(GroupId, deferred) {
    ...
    connection.connect(function (err) {
        ...
        request.execute('proc_UpdateEstimationEGain', function (err, recordsets, returnValue) {
            if (err) {
                logger.info('SP Execution Error on proc_UpdateEstimationEGain');
                closeConnection(connection, 'proc_UpdateEstimationEGain');
                logger.info('ex' + err);
                deferred.reject(err)
            }
            closeConnection(connection, 'proc_UpdateEstimationEGain');
