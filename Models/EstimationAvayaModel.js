...
var EstimationAvayaModel = {

    //Get PricesheetbyOppId
    GetEstimationAvaya: function (oppId) {
        var deferred = q.defer();
        try {
            logger.info('price sheet called')
            if (oppId != null && oppId != undefined) {
                models.EstimationAvaya.findAll({ where: { OppId: oppId } })
            ...
            models.OppApplicationMap.findAll({ where: { ApplicationId: GroupId, OppId: OppId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        ...
                        models.OppApplicationMap.findAll({ where: { OppId: OppId, SheetName: 'AvayaSheet' } })
                            .then(function (opp) {
                                if (opp.length > 0) {
                                    ...
                                    models.sequelize.transaction().then(function (t) {
                                        if (t != null && t != undefined) {
                                            ...
                                            sequelize.query('insert into TBL_EstimationAvaya (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,Fcol4,Fcol5,Fcol6,RowId,GroupId) select :OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.Fcol4,EM.Fcol5,EM.Fcol6,EM.RowId,:groupid from TBL_EstimationAvayaMaster EM left outer join TBL_EstimationAvaya EA on EM.RowId=EA.Rowid and (EA.GroupId = :groupid or EA.GroupId is null)', { replacements: { OppId: OppId, groupid: GroupId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                                .then(function () {
    ...
    GetMaximumGroupEstimationAvayaId: function () {
        var deferred = q.defer();
        try {
    ...
    AddEstimationAvaya: function (EstimationModel) {
        var deferred = q.defer();
        try {
            ...
            models.sequelize.transaction().then(function (t) {
                if (t != null && t != undefined) {
                    if (EstimationModel.IsSaveAs) {
                        models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId, SheetName: 'AvayaSheet', IsEditable: true, IsLocked: true } })
                            .then(function (opp) {
                                if (opp.length > 0) {

                                    if (opp[0].LockedUser == EstimationModel.Authour) {

                                        sequelize.query('update TBL_EstimationOppMapping set IsEditable=0,IsLocked=0,LockedIn=NULL,LockedUser=NULL,LockStartTime=NULL where OppId = :id and SheetName=:sheetname', { replacements: { id: EstimationModel.OppId, sheetname: "AvayaSheet" }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                            .then(function () {
                                                models.OppApplicationMap.create({
                                                    OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'AvayaSheet'
                                                }, { transaction: t })
                                                    .then(function () {
                        ...
                        models.OppApplicationMap.update({
                            ...
                            .then(function () {
                                models.EstimationAvaya.destroy({ where: { GroupId: EstimationModel.ApplicationId }, truncate: false }, { transaction: t })
                                    .then(function () {
                                        Addinternalcall(EstimationModel, t, deferred);
                                    })
    ...
    AddEstimationAvayaNewVersion: function (EstimationModel) {
        var deferred = q.defer();
        try {
                ...
                models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId } })
                    .then(function (opp) {
                        if (opp.length > 0) {
                            ...
                            models.OppApplicationMap.create({
                                OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'AvayaSheet', IsInitial: true
                            }, { transaction: t })
                                .then(function () {
                                    sequelize.query('insert into TBL_EstimationAvaya (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,Fcol4,Fcol5,Fcol6,RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,A1Fcol4,A1Fcol5,A1Fcol6,GroupId) select EA.OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.Fcol4,EM.Fcol5,EM.Fcol6,EM.RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,A1Fcol4,A1Fcol5,A1Fcol6,:groupid from TBL_EstimationAvayaMaster EM left outer join TBL_EstimationAvaya EA on EM.RowId=EA.Rowid  and (EA.GroupId = :oldid or EA.GroupId is null)', { replacements: { oldid: EstimationModel.ExisitingPriceGroupId, groupid: EstimationModel.ApplicationId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                        .then(function () {
                                            t.commit();
    ...
    GetAllEstimationAvayabyOppGroupID: function (oppId, ApplicationId) {
        var deferred = q.defer();
        try {
                ...
                .then(function (opp) {
                    if (opp.length > 0) {
                        if (opp[0].IsEditable) {
                            ...
                            models.EstimationAvaya.findAll({ where: { GroupId: ApplicationId } })
...
module.exports.EstimationAvayaModel = EstimationAvayaModel;
...
function Addinternalcall(ApplicationModel, t, deferred) {
    try {
        ...
        if (t != null && t != undefined) {
            ...
            models.EstimationAvaya.bulkCreate(ApplicationModel.AvayaSheet, { transaction: t }, { omitNull: true }).then(function () {
                t.commit();
...
function UpdateFormuleusingSP(GroupId, deferred) {
    ...
    connection.connect(function (err) {
        ...
        request.execute('proc_UpdateEstimationAvaya', function (err, recordsets, returnValue) {
            if (err) {
                logger.info('SP Execution Error on proc_UpdateEstimationAvaya');
                closeConnection(connection, 'proc_UpdateEstimationAvaya');
                logger.info('ex' + err);
                deferred.reject(err)
            }
            closeConnection(connection, 'proc_UpdateEstimationAvaya');
