...
var EstimationSelfServiceModel = {

    //Get PricesheetbyOppId
    GetEstimationSelfService: function (oppId) {
        var deferred = q.defer();
        try {
            logger.info('price sheet called')
            if (oppId != null && oppId != undefined) {
                models.EstimationSelfService.findAll({ where: { OppId: oppId } })
            ...
            models.OppApplicationMap.findAll({ where: { ApplicationId: GroupId, OppId: OppId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        ...
                        models.OppApplicationMap.findAll({ where: { OppId: OppId, SheetName: 'SelfServiceSheet' } })
                            .then(function (opp) {
                                if (opp.length > 0) {
                                    ...
                                    models.sequelize.transaction().then(function (t) {
                                        if (t != null && t != undefined) {
                                            ...
                                            sequelize.query('insert into TBL_EstimationSelfService (OppId,GroupType,Name,col1,col2,Fcol3,RowId,GroupId) select :OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.Fcol3,EM.RowId,:groupid from TBL_EstimationSelfServiceMaster EM left outer join TBL_EstimationSelfService EA on EM.RowId=EA.Rowid and (EA.GroupId = :groupid or EA.GroupId is null)', { replacements: { OppId: OppId, groupid: GroupId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                                .then(function () {
                                                    ...
                                                    models.OppApplicationMap.create({
                                                        ...
                                                        IsLocked: true,
                                                        IsEditable: true,
                                                        LockedIn: 'SelfServiceSheet',
                                                        SheetName: 'SelfServiceSheet',
                                                        LockedUser: username,
                                                        LockStartTime: localTime,
                                                        CreatedBy: username,
    ...
    GetMaximumGroupEstimationSelfServiceId: function () {
        var deferred = q.defer();
        try {
    ...
    AddEstimationSelfService: function (EstimationModel) {
        var deferred = q.defer();
        try {
            ...
            models.sequelize.transaction().then(function (t) {
                if (t != null && t != undefined) {
                    if (EstimationModel.IsSaveAs) {
                        models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId, SheetName: 'SelfServiceSheet', IsLocked: true } })
                            .then(function (opp) {
                                if (opp.length > 0) {
                                    if (opp[0].LockedUser == EstimationModel.Authour) {

                                        sequelize.query('update TBL_EstimationOppMapping set IsEditable=0,IsLocked=0,LockedIn=NULL,LockedUser=NULL,LockStartTime=NULL where OppId = :id and SheetName=:sheetname', { replacements: { id: EstimationModel.OppId, sheetname: "SelfServiceSheet" }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                            .then(function () {
                                                models.OppApplicationMap.create({
                                                    OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'SelfServiceSheet'
                                                }, { transaction: t })
                                                    .then(function () {
                        ...
                        models.OppApplicationMap.update({
                            ...
                            .then(function () {
                                models.EstimationSelfService.destroy({ where: { GroupId: EstimationModel.ApplicationId }, truncate: false }, { transaction: t })
                                    .then(function () {
                                        Addinternalcall(EstimationModel, t, deferred);
                                    })
    ...
    AddEstimationSelfServiceNewVersion: function (EstimationModel) {
        var deferred = q.defer();
        try {
    ...
    GetAllEstimationSelfServicebyOppGroupID: function (oppId, ApplicationId) {
        var deferred = q.defer();
        ...
        try {
            ...
            models.OppApplicationMap.findAll({ where: { ApplicationId: ApplicationId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        if (opp[0].IsEditable) {
                            sequelize.query(' select ES.[Id], [OppId], ESM.[GroupType], ESM.[Name], ESM.[col1], ESM.[col2], ESM.[Fcol3], [A1col1], [A1col2], [A1Fcol3], [A2col1], [A2col2], [A2Fcol3], [A3col1], [A3col2], [A3Fcol3], [A4col1], [A4col2], [A4Fcol3], [A5col1], [A5col2], [A5Fcol3], [A6col1], [A6col2], [A6Fcol3], [A7col1], [A7col2], [A7Fcol3], [A8col1], [A8col2], [A8Fcol3], [A9col1], [A9col2], [A9Fcol3], [A10col1], [A10col2], [A10Fcol3], ESM.[RowId], [GroupId] from  TBL_EstimationSelfServiceMaster ESM left outer join [TBL_EstimationSelfService] ES on Es.RowId = ESM.RowId where GroupId= :groupid or GroupId is null', { replacements: { groupid: ApplicationId }, type: sequelize.QueryTypes.SELECT })
                            ...
                            models.EstimationSelfService.findAll({ where: { GroupId: ApplicationId } })
...
module.exports.EstimationSelfServiceModel = EstimationSelfServiceModel;
...
function Addinternalcall(ApplicationModel, t, deferred) {
    try {
        ...
        if (t != null && t != undefined) {
            ...
            models.EstimationSelfService.bulkCreate(ApplicationModel.SelfServiceSheet, { transaction: t }, { omitNull: true }).then(function () {
                t.commit();
...
function UpdateFormuleusingSP(GroupId, deferred) {
    ...
    connection.connect(function (err) {
        ...
        request.execute('proc_UpdateEstimationSelfService', function (err, recordsets, returnValue) {
            if (err) {
                logger.info('SP Execution Error on proc_UpdateEstimationSelfService');
                closeConnection(connection, 'proc_UpdateEstimationSelfService');
                logger.info('ex' + err);
                deferred.reject(err)
            }
            closeConnection(connection, 'proc_UpdateEstimationSelfService');
