...
var EstimationAcqueonModel = {

    //Get PricesheetbyOppId
    GetEstimationAcqueon: function (oppId) {
        var deferred = q.defer();
        try {
            logger.info('price sheet called')
            if (oppId != null && oppId != undefined) {
                models.EstimationAcqueon.findAll({ where: { OppId: oppId } })
            ...
            models.OppApplicationMap.findAll({ where: { ApplicationId: GroupId, OppId: OppId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        ...
                        models.OppApplicationMap.findAll({ where: { OppId: OppId, SheetName: 'AcqueonSheet' } })
                            .then(function (opp) {
                                if (opp.length > 0) {
                                    ...
                                    models.sequelize.transaction().then(function (t) {
                                        if (t != null && t != undefined) {
                                            ...
                                            sequelize.query('insert into TBL_EstimationAcqueon (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,Fcol4,Fcol5,Fcol6,RowId,GroupId) select :OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.Fcol4,EM.Fcol5,EM.Fcol6,EM.RowId,:groupid from TBL_EstimationAcqueonMaster EM left outer join TBL_EstimationAcqueon EA on EM.RowId=EA.Rowid and (EA.GroupId = :groupid or EA.GroupId is null)', { replacements: { OppId: OppId, groupid: GroupId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                                .then(function () {
                                                    ...
                                                    models.OppApplicationMap.create({
                                                        ...
                                                        IsLocked: true,
                                                        IsEditable: true,
                                                        LockedIn: 'AcqueonSheet',
                                                        SheetName: 'AcqueonSheet',
                                                        LockedUser: username,
                                                        LockStartTime: localTime,
                                                        CreatedBy: username,
    ...
    GetMaximumGroupEstimationAcqueonId: function () {
        var deferred = q.defer();
        try {
    ...
    AddEstimationAcqueon: function (EstimationModel) {
        var deferred = q.defer();
        try {
            ...
            models.sequelize.transaction().then(function (t) {
                if (t != null && t != undefined) {
                    if (EstimationModel.IsSaveAs) {
                        models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId, SheetName: 'AcqueonSheet', IsEditable: true, IsLocked: true } })
                            .then(function (opp) {

                                if (opp.length > 0) {
                                    if (opp[0].LockedUser == EstimationModel.Authour) {

                                        sequelize.query('update TBL_EstimationOppMapping set IsEditable=0,IsLocked=0,LockedIn=NULL,LockedUser=NULL,LockStartTime=NULL where OppId = :id and SheetName=:sheetname', { replacements: { id: EstimationModel.OppId, sheetname: "AcqueonSheet" }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                            .then(function () {
                                                models.OppApplicationMap.create({
                                                    OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'AcqueonSheet'
                                                }, { transaction: t })
                                                    .then(function () {
                        ...
                        models.OppApplicationMap.update({
                            ...
                            .then(function () {
                                models.EstimationAcqueon.destroy({ where: { GroupId: EstimationModel.ApplicationId }, truncate: false }, { transaction: t })
                                    .then(function () {
                                        Addinternalcall(EstimationModel, t, deferred);
                                    })
    ...
    AddEstimationAcqueonNewVersion: function (EstimationModel) {
        var deferred = q.defer();
        try {
                ...
                models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId } })
                    .then(function (opp) {
                        if (opp.length > 0) {
                            ...
                            models.OppApplicationMap.create({
                                OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'AcqueonSheet', IsInitial: true
                            }, { transaction: t })
                                .then(function () {
                                    sequelize.query('insert into TBL_EstimationAcqueon (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,Fcol4,Fcol5,Fcol6,RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,A1col4,A1Fcol1,A1Fcol5,A1Fcol6,GroupId) select EA.OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.Fcol4,EM.Fcol5,EM.Fcol6,EM.RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,A1col4,A1Fcol1,A1Fcol5,A1Fcol6,:groupid from TBL_EstimationAcqueonMaster EM left outer join TBL_EstimationAcqueon EA on EM.RowId=EA.Rowid  and (EA.GroupId = :oldid or EA.GroupId is null)', { replacements: { oldid: EstimationModel.ExisitingPriceGroupId, groupid: EstimationModel.ApplicationId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                        .then(function () {
                                            t.commit();
    ...
    GetAllEstimationAcqueonbyOppGroupID: function (oppId, ApplicationId) {
        var deferred = q.defer();
        try {
            models.OppApplicationMap.findAll({ where: { ApplicationId: ApplicationId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        if (opp[0].IsEditable) {
                            sequelize.query('SELECT EA.[Id],[OppId],EAM.[GroupType],EAM.[Name],EAM.[col1],EAM.[col2],EAM.[col3],EAM.[Name1],EAM.[Fcol1],EAM.[Fcol2],EAM.[Fcol3],EAM.[Fcol4],EAM.[Fcol5],EAM.[Fcol6],EAM.[RowId],[A1col1],[A1col2],[A1col3],[A1Fcol1],[A1Fcol2],[A1Fcol3],[A1Fcol4],[A1Fcol5],[A1Fcol6],[GroupId]  FROM  TBL_EstimationAcqueonMaster EAM left outer join [dbo].[TBL_EstimationAcqueon] EA  on EA.RowId=EAM.RowId where GroupId= :groupid or GroupId is null', { replacements: { groupid: ApplicationId }, type: sequelize.QueryTypes.SELECT })
                            ...
                            models.EstimationAcqueon.findAll({ where: { GroupId: ApplicationId } })
...
module.exports.EstimationAcqueonModel = EstimationAcqueonModel;
...
function Addinternalcall(ApplicationModel, t, deferred) {
    try {
        ...
        if (t != null && t != undefined) {
            ...
            models.EstimationAcqueon.bulkCreate(ApplicationModel.AcqueonSheet, { transaction: t }, { omitNull: true }).then(function () {
                t.commit();
...
function UpdateFormuleusingSP(GroupId, deferred) {
    ...
    connection.connect(function (err) {
        ...
        request.execute('PROC_UpdateEstimationAcqueon', function (err, recordsets, returnValue) {
            if (err) {
                logger.info('SP Execution Error on PROC_UpdateEstimationAcqueon');
                closeConnection(connection, 'PROC_UpdateEstimationAcqueon');
                logger.info('ex' + err);
                deferred.reject(err)
            }
            closeConnection(connection, 'PROC_UpdateEstimationAcqueon');
