...
var EstimationWFOModel = {

    //Get PricesheetbyOppId
    GetEstimationWFO: function (oppId) {
        var deferred = q.defer();
        try {
            logger.info('price sheet called')
            if (oppId != null && oppId != undefined) {
                models.EstimationWFO.findAll({ where: { OppId: oppId } })
            ...
            models.OppApplicationMap.findAll({ where: { ApplicationId: GroupId, OppId: OppId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        ...
                        models.OppApplicationMap.findAll({ where: { OppId: OppId, SheetName: 'WFOSheet' } })
                            .then(function (opp) {
                                if (opp.length > 0) {
                                    ...
                                    models.sequelize.transaction().then(function (t) {
                                        if (t != null && t != undefined) {
                                            ...
                                            sequelize.query('insert into TBL_EstimationWFO (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,GroupId) select :OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,:groupid from TBL_EstimationWFOMaster EM left outer join TBL_EstimationWFO EA on EM.RowId=EA.Rowid and (EA.GroupId = :groupid or EA.GroupId is null)', { replacements: { OppId: OppId, groupid: GroupId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                                .then(function () {
    ...
    GetMaximumGroupEstimationWFOId: function () {
        var deferred = q.defer();
        try {
    ...
    AddEstimationWFO: function (EstimationModel) {
        var deferred = q.defer();
        try {
            ...
            models.sequelize.transaction().then(function (t) {
                if (t != null && t != undefined) {
                    if (EstimationModel.IsSaveAs) {
                        models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId, SheetName: 'WFOSheet', IsLocked: true } })
                            .then(function (opp) {
                                if (opp.length > 0) {

                                    if (opp[0].LockedUser == EstimationModel.Authour) {

                                        sequelize.query('update TBL_EstimationOppMapping set IsEditable=0,IsLocked=0,LockedIn=NULL,LockedUser=NULL,LockStartTime=NULL where OppId = :id and SheetName=:sheetname', { replacements: { id: EstimationModel.OppId, sheetname: "WFOSheet" }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                            .then(function () {
                                                models.OppApplicationMap.create({
                                                    OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'WFOSheet'
                                                }, { transaction: t })
                                                    .then(function () {
                        ...
                        models.OppApplicationMap.update({
                            ...
                            .then(function () {
                                models.EstimationWFO.destroy({ where: { GroupId: EstimationModel.ApplicationId }, truncate: false }, { transaction: t })
                                    .then(function () {
                                        Addinternalcall(EstimationModel, t, deferred);
                                    })
    ...
    AddEstimationWFONewVersion: function (EstimationModel) {
        var deferred = q.defer();
        try {
                ...
                models.OppApplicationMap.findAll({ where: { OppId: EstimationModel.OppId } })
                    .then(function (opp) {
                        if (opp.length > 0) {
                            ...
                            models.OppApplicationMap.create({
                                OppId: EstimationModel.OppId, ApplicationId: EstimationModel.ApplicationId, Version: EstimationModel.Version, IsEditable: EstimationModel.IsReadOnly, CreatedBy: EstimationModel.Authour, Comment: EstimationModel.Comment, LockedIn: '', NumberOfApplication: EstimationModel.NumberofApp, SheetName: 'WFOSheet', IsInitial: true
                            }, { transaction: t })
                                .then(function () {
                                    sequelize.query('insert into TBL_EstimationWFO (OppId,GroupType,Name,col1,col2,col3,Name1,Fcol1,Fcol2,Fcol3,RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,GroupId) select EA.OppId,EM.GroupType,EM.Name,EM.col1,EM.col2,EM.col3,EM.Name1,EM.Fcol1,EM.Fcol2,EM.Fcol3,EM.RowId,A1col1,A1col2,A1col3,A1Fcol1,A1Fcol2,A1Fcol3,:groupid from TBL_EstimationWFOMaster EM left outer join TBL_EstimationWFO EA on EM.RowId=EA.Rowid  and (EA.GroupId = :oldid or EA.GroupId is null)', { replacements: { oldid: EstimationModel.ExisitingPriceGroupId, groupid: EstimationModel.ApplicationId }, type: sequelize.QueryTypes.SELECT, transaction: t })
                                        .then(function () {
                                            t.commit();
    ...
    GetAllEstimationWFObyOppGroupID: function (oppId, ApplicationId) {
        var deferred = q.defer();
        try {
            models.OppApplicationMap.findAll({ where: { ApplicationId: ApplicationId } })
                .then(function (opp) {
                    if (opp.length > 0) {
                        if (opp[0].IsEditable) {
                              sequelize.query('SELECT WF.[Id],[OppId],WFM.[GroupType],WFM.[Name],WFM.[col1],WFM.[col2],WFM.[col3],WFM.[Name1],WFM.[Fcol1],WFM.[Fcol2],WFM.[Fcol3],WFM.[RowId],[A1col1],[A1col2],[A1col3],[A1Fcol1],[A1Fcol2],[A1Fcol3],[GroupId]  FROM TBL_EstimationWFOMaster WFM  left outer join [dbo].[TBL_EstimationWFO] WF  on WF.RowId=WFM.RowId where GroupId= :groupid or GroupId is null', { replacements: { groupid: ApplicationId }, type: sequelize.QueryTypes.SELECT })
                            ...
                            models.EstimationWFO.findAll({ where: { GroupId: ApplicationId } })
...
module.exports.EstimationWFOModel = EstimationWFOModel;
...
function Addinternalcall(ApplicationModel, t, deferred) {
    try {
        ...
        if (t != null && t != undefined) {
            ...
            models.EstimationWFO.bulkCreate(ApplicationModel.WFOSheet, { transaction: t }, { omitNull: true }).then(function () {
                t.commit();
...
function UpdateFormuleusingSP(GroupId, deferred) {
    ...
    connection.connect(function (err) {
        ...
        request.execute('PROC_UpdateEstimationWFO', function (err, recordsets, returnValue) {
            if (err) {
                logger.info('SP Execution Error on PROC_UpdateEstimationWFO');
                closeConnection(connection, 'PROC_UpdateEstimationWFO');
                logger.info('ex' + err);
                deferred.reject(err)
            }
            closeConnection(connection, 'PROC_UpdateEstimationWFO');
